...
if ($reg=mysqli_fetch_array($registros))
{
    ...
    <div class="col-sm-8">
        <p align="center"><a href="#demo" class="btn btn-info" data-toggle="collapse">Datos Perfil</a></p>
...
while ($reg=mysqli_fetch_array($hora))
{
        ...
        $horaid=$reg['id'];
        $dia=1;
        $horario=mysqli_query($conexion,"SELECT * FROM horario AS ho INNER JOIN materia AS ma ON ho.id_materia=ma.id INNER JOIN nivel AS ni ON ho.id_nivel=ni.id WHERE ho.cedula_docente='$_REQUEST[id]' and ho.id_dia='$dia' and ho.id_hora='$horaid'") or
                die("Problemas en el select:".mysqli_error($conexion));

                if ($h=mysqli_fetch_array($horario))
                {
                    echo utf8_encode($h['nombre'].'<br>'.$h['curso'].' '.$h['paralelo']);
