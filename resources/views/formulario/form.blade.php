...
<div class="form-group {{ $errors->has('categoria') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="categoria" required="true" style="font-size:12px" type="text" id="categoria" value="{{ isset($formulario->categoria) ? $formulario->categoria : ''}}">
    {!! $errors->first('ca', 'tegoria<p class="help-block">:message</p>') !!}
</div>
<div class="form-group {{ $errors->has('descripcion') ? 'has-error' : ''}}">
    <label for="descripcion" class="control-label">{{ 'Descripcion' }}</label>
    <textarea class="form-control" rows="5" name="descripcion" style="font-size:12px" required="true" type="textarea" id="descripcion" >{{ isset($formulario->descripcion) ? $formulario->descripcion : ''}}</textarea>
